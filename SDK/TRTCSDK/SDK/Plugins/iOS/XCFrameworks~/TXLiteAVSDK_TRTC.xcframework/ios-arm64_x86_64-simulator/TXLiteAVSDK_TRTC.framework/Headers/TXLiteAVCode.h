...
enum TXLiteAVError {
    ...
    /// 开始录屏失败，如果在移动设备出现，可能是权限被用户拒绝了。在 Windows 或 Mac 系统的设备出现，请检查录屏接口的参数是否符合要求
    ...
    /// 系统混音启动出错，常见的出错原因及解决：1.当前系统版本无法支持，指定、排除指定进程混音功能需满足 Windows 系统为10.0.19042（20H2）或更高版本，指定路径混音功能 x64 版本需满足 Windows
    /// 系统为10.0.19042（20H2）或更高版本，请检查当前系统版本。2.传入参数非法（包括参数为空、传入路径不存在、传入进程号不存在、传入设备名不存在），请检查传入参数是否合法。3.无可用扬声器设备，请检查是否可用
    ERR_SYSTEM_LOOPBACK_START_FAILED = -1307,

    /// 第三方应用混音采集出错，该错误可能出现于第三方应用混音启动成功后，在采集声音的过程中指定进程或应用意外退出，请检查指定进程或应用的状态，可通过重新输入进程号进行重试
    ...
    /// 非法路径导致打开文件失败，请检查您传入的路径参数是否指向一个合法的音乐文件
    ERR_BGM_INVALID_PATH = -4005,

    /// 非法URL导致打开文件失败，请用浏览器检查您传入的 URL 地址是否可以下载到期望的音乐文件，如果操作系统为 iOS 或 MacOS 请确保使用 https 链接
    ERR_BGM_INVALID_URL = -4006,

    /// 无音频流导致打开文件失败，请确认您传入的文件是否是合法的音频文件，以及文件是否被损坏
    ERR_BGM_NO_AUDIO_STREAM = -4007,

    /// 格式不支持导致打开文件失败，请确认您传入的文件格式是否是支持的文件格式，移动端支持【mp3，aac，m4a，wav，ogg，mp4，mkv】，桌面端支持 【mp3，aac，m4a，wav，mp4，mkv】
