 ...
 */
     ...
     * 这个字段，也就是将房间中的用户区分为 **主播** ({@link TRTCRoleAnchor}) 和 **观众** ({@link TRTCRoleAudience}) 两种不同的角色。 调用该接口后，您会收到来自 {@link ITRTCCloudCallback} 中的 {@link onEnterRoom} 回调：
     ...
     * 接口调用结果会通过 {@link ITRTCCloudCallback} 中的 {@link onSwitchRoom} 回调。
     ...
     * 3. 您可以分别为不同实例分别设置 {@link ITRTCCloudCallback} 获取各自的事件通知。
 ...
 * 当开始渲染首帧摄像头画面时，您会收到 {@link ITRTCCloudCallback} 中的 {@link onCameraDidReady} 回调通知。
 ...
 */
     ...
     * 5.12 启用音量大小提示。
     *
     * 开启此功能后，SDK 会在 {@link ITRTCCloudCallback} 中的 {@link onUserVoiceVolume} 回调中反馈本地和远端用户的音频音量评估信息，包括音量大小、人声检测、音频频谱等。
     ...
     * 本次录制的启动、完成状态会通过本地录制相关回调进行通知。参见 {@link ITRTCCloudCallback} 相关回调。
     * @param param 录音参数，请参见 {@link TRTCAudioRecordingParams}。
     * @return 0：成功；-1：录音已开始；-2：文件或目录创建失败；-3：后缀指定的音频格式不支持。
     * @note 自 v11.5 版本，音频录制的状态结果由返回值统一调整为异步回调进行通知。参见 {@link ITRTCCloudCallback} 相关回调。
     */
 ...
 */
#if TARGET_PLATFORM_DESKTOP || __ANDROID__
 ...
 */
     ...
     * 房间中的其他用户可以通过 {@link ITRTCCloudCallback} 中的 {@link onRecvSEIMsg} 回调接收消息。
