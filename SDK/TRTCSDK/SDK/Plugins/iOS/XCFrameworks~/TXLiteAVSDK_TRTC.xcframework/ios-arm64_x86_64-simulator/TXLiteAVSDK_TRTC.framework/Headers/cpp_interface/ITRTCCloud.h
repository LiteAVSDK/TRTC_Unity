 ...
 * @note
 * 1. If you use `delete ITRTCCloud*`, a compilation error will occur. Please use {@link destroyTRTCShareInstance} to release the object pointer.
 ...
 */
     ...
     * - Manual subscription: after room entry, the user needs to manually call the {@link startRemoteView} API to start subscribing to and decoding the video stream and call the {@link muteRemoteAudio} ($NO$) API to start playing back the audio
     ...
     * - After that, the audio stream of anchor C will no longer be published to room "101", and both anchor A and audience B will receive the `onUserAudioAvailable(C,$NO$)` event callback, and cannot subscribe to the audio stream of anchor C by
     * calling `muteRemoteAudio(C,$NO$)`.
 ...
 */
     ...
     * @note This API can be called before room entry ({@link enterRoom}), and the pause status will be reset after room exit ({@link exitRoom}). After calling this API to pause receiving the video stream from a specific user, simply calling the
     * {@link startRemoteView} API will not be able to play the video from that user. You need to call {@link muteRemoteVideoStream}($NO$) or {@link muteAllRemoteVideoStreams}($NO$) to resume it.
     */
     ...
     * muteRemoteVideoStream}($NO$) or {@link muteAllRemoteVideoStreams}($NO$) to resume it.
     */
 ...
 */
#if _WIN32 || __APPLE__ || __OHOS__ || __ANDROID__ || __linux__
     ...
     */
    virtual ITXBeautyManager* getBeautyManager() = 0;
     ...
     * @param streamType Stream type of the watermark to be set ({@link $TRTCVideoStreamTypeBig$} or @@link $TRTCVideoStreamTypeSub$})
     * @param srcData    Source data of watermark image (if `nullptr` is passed in, the watermark will be removed)
     * @param srcType    Source data type of watermark image. For more information, please see {@link TRTCWaterMarkSrcType}
 ...
 * @param deviceName If this parameter is empty, the audio of the entire system is captured.
 * @note On the Windows platform, you can specify the parameter `deviceName` to the absolute path of an executable file (such as `QQMuisc.exe`) of a certain application. In this case, the SDK will only capture the sound of that application (32-bit
 * version of the SDK is supported, 64-bit version of the SDK requires Windows version 10.0.19042 or higher). You can also specify `deviceName` as the name of a certain speaker device to capture specific speaker sound (you can use the getDevicesList
 * interface in {@link TXDeviceManager} to obtain the speaker devices of type {@link TXMediaDeviceTypeSpeaker}). On the Windows platform, you can also specify `deviceName` as the process ID of a certain process (in the format of "process_xxx", where
 * xxx is the process ID), and then the SDK will capture the sound of that process (requires Windows version 10.0.19042 or higher). Alternatively, on the Windows platform, you can specify `deviceName` as the process ID of a certain process to be
 * excluded (in the format of "exclude_process_xxx", where xxx is the process ID), and then the SDK will capture all sounds except for that process (requires Windows version 10.0.19042 or higher). About speaker device name you can see {@link
 * TXDeviceManager}
 */
#if TARGET_PLATFORM_DESKTOP || __ANDROID__ || __OHOS__
 ...
 */
#if TARGET_PLATFORM_DESKTOP || TARGET_OS_IPHONE || __OHOS__
     ...
     */
    virtual AITranscriberManager* getAITranscriberManager() = 0;
...
/**
 * 10.5 Enumerate shareable screens and windows (for desktop systems only).
 *
 ...
 * 2. Please do not use `delete ITRTCScreenCaptureSourceList*` to delete the `SourceList`; otherwise, crashes may occur. Instead, please use the `release` method in {@link ITRTCScreenCaptureSourceList} to release the list.
 */
#if TARGET_PLATFORM_DESKTOP
...
/**
 * 10.6 Select the screen or window to share (for desktop systems only).
 *
 * After you get the sharable screens and windows through {@link getScreenCaptureSources}, you can call this API to select the target screen or window you want to share.
 ...
 * - Sharing the entire screen: for `source` whose `type` is {@link TRTCScreenCaptureSourceTypeScreen}  in `sourceInfoList`, set `captureRect` to `{ 0, 0, 0, 0 }`.
 * - Sharing a specified area: for `source` whose `type` is {@link TRTCScreenCaptureSourceTypeScreen}  in `sourceInfoList`, set `captureRect` to a non-nullptr value, e.g., `{ 100, 100, 300, 300 }`.
 * - Sharing an entire window: for `source` whose `type` is {@link TRTCScreenCaptureSourceTypeWindow} in `sourceInfoList`, set `captureRect` to `{ 0, 0, 0, 0 }`.
 * - Sharing a specified window area: for `source` whose `type` is {@link TRTCScreenCaptureSourceTypeWindow} in `sourceInfoList`, set `captureRect` to a non-nullptr value, e.g., `{ 100, 100, 300, 300 }`.
 ...
 */
    ...
    /**
     * 11.9.1 Enable third-party beauty filters in video.
     *
    ...
    /**
     * 11.9.2 Set video data callback for third-party beauty filters.
     *
     ...
     * @param pixelFormat Specify the format of the pixel called back
     * @param bufferType  Specify video data structure type.
