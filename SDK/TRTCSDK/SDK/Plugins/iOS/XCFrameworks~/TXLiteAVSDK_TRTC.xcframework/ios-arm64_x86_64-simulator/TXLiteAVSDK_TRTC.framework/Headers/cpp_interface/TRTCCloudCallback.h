...
class ITRTCCloudCallback {
   public:
     ...
     *
     * You can call the {@link updateOtherRoomForwardMode} API in `TRTCCloud` to limit the upstream capability of the cross-room anchor in the current room, and prohibit or allow the cross-room anchor to publish audio/video/sub-stream. This behavior
     * will affect all users in the room. The caller will receive the `onUpdateOtherRoomForwardMode()` callback to determine whether the changing upstream capability of the cross-room anchor is successful.
 ...
 */
    ...
    virtual void onAudioRouteChanged(TRTCAudioRoute newRoute, TRTCAudioRoute oldRoute) {
    }
#endif
 ...
 * @param deviceType Device type. `0`: microphone; `1`: speak; `2`: camera
 ...
 */
     ...
     * If the code return value is ERR_SERVER_PROCESS_FAILED, it means the server failed to process your request. You can use "error_code" as the key to retrieve the error code returned by the server. The specific error codes and recommended actions
     * are as follows:
     ...
     * @param message: The callback information.
     * @param extraInfo: Additional information. For some error codes, there may be additional information to help you troubleshoot the issues.
     * If the code return value is ERR_SERVER_PROCESS_FAILED, it means the server failed to process your request. You can use "error_code" as the key to retrieve the error code returned by the server. The specific error codes and recommended
     ...
     * @param message: The callback information.
     * @param extraInfo: Additional information. For some error codes, there may be additional information to help you troubleshoot the issues.
     * If the code return value is ERR_SERVER_PROCESS_FAILED, it means the server was unable to process your request. You can use "error_code" as the key to retrieve the error code returned by the server. The specific error codes and recommended
 ...
 */
     ...
     * @param format Screenshot data format. Only {@link TRTCVideoPixelFormat_BGRA32} is supported now.
     ...
     */
    virtual void onExperimentalEvent(const char* eventName, const char* params) {
    }
