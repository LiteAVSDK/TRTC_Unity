 ...
 * 从 11.4.0 版本以后，我们推荐您使用功能更加强大的{@link addDelegate}接口，支持添加/移除多个监听。
 */
...
/**
 * 1.6 设置驱动 TRTCCloudDelegate 事件回调的队列
 *
 ...
 * @note 调用该接口暂停接收指定用户的视频流后，只调用 {@link startRemoteView} 接口无法播放指定用户的视频，需要调用 {@link muteRemoteVideoStream}(NO) 或 {@link muteAllRemoteVideoStreams}(NO) 才能恢复。
 */
 ...
 * @param mute  是否暂停接收。
 * @note 该接口支持您在进入房间（{@link enterRoom}）前调用，暂停状态在退出房间（{@link exitRoom}）之后会被重置。
 * @note 调用该接口暂停接收所有用户的视频流后，只调用 {@link startRemoteView} 接口无法播放指定用户的视频，需要调用 {@link muteRemoteVideoStream}(NO) 或 {@link muteAllRemoteVideoStreams}(NO) 才能恢复。
 */
 ...
 * @param srcPoints 原始画面坐标区域4个顶点坐标，需按照左上、左下、右上、右下的顺序传入，所有的坐标需要根据画面宽高做 [0,1] 区间的归一化; 传 null 则停止透视校正。
 * @param dstPoints 期望校正到的目标坐标区域4个顶点坐标，需按照左上、左下、右上、右下的顺序传入，所有的坐标需要根据画面宽高做 [0,1] 区间的归一化；传 null 则停止透视校正。
 */
 ...
 * @param route 音频路由，即声音由哪里输出（扬声器、听筒），默认值：{@link TRTCAudioModeSpeakerphone}。
 */
