...
/**
 * 开启自定义Dns解析，开启后可避免 Local DNS 造成域名劫持和跨网访问问题，解决移动互联网服务中域名解析异常带来的困扰。
 * 需实现Dns解析代理接口 {@link TXLiveBaseDelegate#onCustomHttpDNS}
 * 代码使用示例：
 * [TXLiveBase enableCustomHttpDNS:YES];
 * #pragma mark -TXLiveBaseDelegate
 * - (void)onCustomHttpDNS:(NSString *)hostName ipList:(NSMutableArray<NSString *> *)list {
 *   // 举例：使用腾讯云移动解析HTTPDNS(https://cloud.tencent.com/document/product/379) 进行域名解析
 *   // NSArray *result = [[MSDKDns sharedInstance] WGGetHostByName:hostName];
 *   // ......
 *   // [list addObject:ip];
 * }
 * @param enable - YES: 开启自定义解析Dns
 *          - NO: 关闭自定义解析Dns，默认关闭
 */
