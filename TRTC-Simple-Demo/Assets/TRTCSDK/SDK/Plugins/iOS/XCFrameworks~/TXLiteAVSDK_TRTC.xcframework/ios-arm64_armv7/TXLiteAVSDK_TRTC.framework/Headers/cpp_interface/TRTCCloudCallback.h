...
class ITRTCCloudCallback {
   public:
     ...
     * 2.1 进入房间成功与否的事件回调。
     *
     * 调用 TRTCCloud 中的 {@link enterRoom} 接口执行进房操作后，会收到来自 {@link ITRTCCloudCallback} 的 `onEnterRoom(result)` 回调：
     ...
     * 2.7 更改跨房主播上行能力的结果回调。
     *
     * 调用 TRTCCloud 中的 {@link updateOtherRoomForwardMode} 接口会限制跨房主播在本房间内的上行能力，禁止或允许跨房主播发布音频/主路视频/辅路视频，该行为会影响房间内的所有用户。
     ...
     * 3.12 用户视频大小发生改变回调。
     *
     * 当您收到 `onUserVideoSizeChanged(userId, streamType, newWidth, newHeight)` 通知时，表示该路画面大小发生了调整，调整的原因可能是该用户调用了 {@link setVideoEncoderParam} 或者 {@link setSubStreamEncoderParam} 重新设置了画面尺寸。
     * @param userId 用户标识。
     * @param streamType 视频流类型：主路（Main）一般用于承载摄像头画面，辅路（Sub）一般用于承载屏幕分享画面。
 ...
 * @param err ERR_NULL 表示成功，其余值表示失败，详见[错误码](https://cloud.tencent.com/document/product/647/32257)。
 */
#if TARGET_PLATFORM_DESKTOP
 ...
 */
     ...
     * 音效播放已结束（已废弃）。
     *
     * @deprecated 新版本开始不推荐使用，建议使用 {@link TXAudioEffectManager} 接口替代之。
