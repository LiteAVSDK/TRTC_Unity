...
typedef void *TXView;
#endif
typedef TXImageBuffer TRTCImageBuffer;
...
enum TRTCVideoBufferType {
    ...
    /// Use memory buffer to transfer video data. iOS: memory data block; Android: `Direct Buffer` for JNI layer; Windows: memory data block.
    ...
    /// Use `PixelBuffer` to transfer video data, only for iOS.
...
enum TRTCTranscodingConfigMode {
    ...
    ///- You need to listen on the {@link onUserVideoAvailable} and {@link onUserAudioAvailable} event callbacks in {@link ITRTCCloudCallback} and constantly adjust the `mixUsers` parameter according to the audio/video status of each user with mic on
    /// in the current room.
    ...
    /// In this mode, you don't need to listen on the {@link onUserVideoAvailable} and {@link onUserAudioAvailable} callbacks in {@link ITRTCCloudCallback} to make real-time adjustments.
...
struct TRTCVideoFrame {
    ...
    /// Field description: video data.
    /// - When `bufferType` is {@link TRTCVideoBufferType_Buffer}, carries the memory data blocks for the C++ layer.
    ///- When `bufferType` is {@link TRTCVideoBufferType_PixelBuffer}, carries the `PixelBuffer` unique to iOS for the C++ layer.
    char *data;
